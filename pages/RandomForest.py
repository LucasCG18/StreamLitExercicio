...
st.set_page_config(layout="wide", page_title="RandomForest - Breast Cancer")
st.title("Modelo: RandomForest")
...
n_estimators = st.slider("Árvores (RandomForest)", min_value=50, max_value=500, value=200, step=50)
train_btn = st.button("Treinar RandomForest")
...
st.divider()
st.subheader("Prever um novo caso (RandomForest)")
feat_cols = st.session_state.get('rf_features', selected_features)
if not feat_cols:
    ...
    with st.form("rf_predict_form"):
        ...
        run_pred = st.form_submit_button("Prever com RandomForest")

    if run_pred:
        if 'rf_model' not in st.session_state or 'rf_imputer' not in st.session_state:
            st.warning("Treine o modelo primeiro.")
        else:
            imputer: SimpleImputer = st.session_state['rf_imputer']
            model: RandomForestClassifier = st.session_state['rf_model']
